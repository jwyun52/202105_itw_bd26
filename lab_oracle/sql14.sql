...
create table customers (
    ...
    cust_gender number(1) default 0
                ...
                -- constraint ck_cust_gender check (cust_gender = 0 or cust_gender = 1 or cust_gender = 2)
    cust_age    number(3) default 0
                constraint ck_cust_age check (cust_age between 0 and  200)
                -- constraint ck_cust_age check (cust_age >= 0 and  cust_age <= 200)
);
...
create table customers2 (
    ...
    constraint ck_pw2 check (cust_pw is not null and lengthb(cust_pw) >= 8),
