...
test_idx <- idx[121:150]  # 30개 랜덤 인덱스

train_features <- features[train_idx, ]  # 훈련 특성
test_features <- features[test_idx, ]  # 테스트 특성
...
# 테스트 셋에서의 정확도, 오차 행렬 계산 -> 평가
test_predictions <- predict(tree_clf,       # 훈련된 머신러닝 모델
                            test_features)  # 테스트 특성
mean(test_predictions == test_target)  # 93.3%
